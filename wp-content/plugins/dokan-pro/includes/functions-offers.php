...
/**
 * Returns the definitions for the offers and charts
 *
 * @since 1.0
 *
 * @return array
 */
function dokan_get_offers_charts() {
    $charts = [
        ...
        'charts' => [
            'overview'        => [
                ...
                'permission'  => 'dokan_view_overview_offer',
            ],
            'sales_by_day'    => [
                'title'       => __( 'Sales by day', 'dokan' ),
                'description' => '',
                'function'    => 'dokan_daily_sales',
                'permission'  => 'dokan_view_daily_sale_offer',
            ],
            'top_sellers'     => [
                'title'       => __( 'Top selling', 'dokan' ),
                'description' => '',
                'function'    => 'dokan_top_sellers',
                'permission'  => 'dokan_view_top_selling_offer',
            ],
            'top_earners'     => [
                'title'       => __( 'Top earning', 'dokan' ),
                'description' => '',
                'function'    => 'dokan_top_earners',
                'permission'  => 'dokan_view_top_earning_offer',
            ],
             'sales_statement' => [
                'title'       => __( 'Statement', 'dokan' ),
                'description' => '',
                'function'    => 'dokan_seller_sales_statement',
                'permission'  => 'dokan_view_statement_offer',
            ],
        ],
    ];

    return apply_filters( 'dokan_offers_charts', $charts );
}
...
function dokan_seller_sales_statement() {
    ...
    if ( isset( $_GET['dokan_offer_filter'] ) ) {
        $start_date = date( 'Y-m-d', strtotime( $_GET['start_date'] ) );
        $end_date   = date( 'Y-m-d', strtotime( $_GET['end_date'] ) );
    } ?>

    <form method="get" class="dokan-form-inline offer-filter dokan-clearfix" action="">
        ...
        <div class="dokan-form-group">
            ...
            <input type="submit" name="dokan_offer_filter" class="dokan-btn dokan-btn-success dokan-btn-sm dokan-theme" value="<?php _e( 'Show', 'dokan' ); ?>" />
        </div>
...
/**
 * Generate SQL query and fetch the offer data based on the arguments passed
 *
 * This function was cloned from WC_Admin_offer class.
 *
 * @since 1.0
 *
 * @global WPDB $wpdb
 ...
 * @return obj
 */
function dokan_get_order_offer_data( $args = [], $start_date, $end_date, $current_user = false ) {
    global $wpdb;
    ...
    $query      = apply_filters( 'dokan_offers_get_order_offer_query', $query );
    $query      = implode( ' ', $query );
    ...
    if ( $debug || $nocache || ( false === ( $result = get_transient( 'dokan_wc_offer_' . $query_hash ) ) ) ) {
        $result = apply_filters( 'dokan_offers_get_order_offer_data', $wpdb->$query_type( $query ), $data );
        ...
        set_transient( 'dokan_wc_offer_' . $query_hash, $result, $expiration );
    }
...
/**
 * Generate sales overview offer chart in offer area
 *
 * @since 1.0
 *
 * @return void
...
function dokan_sales_overview() {
    ...
    dokan_offer_sales_overview( $start_date, $end_date, __( 'This month\'s sales', 'dokan' ) );
}
...
function dokan_daily_sales() {
    ...
    if ( isset( $_POST['dokan_offer_filter'] ) ) {
        $start_date = date( 'Y-m-d', strtotime( $_POST['start_date'] ) );
        $end_date   = date( 'Y-m-d', strtotime( $_POST['end_date'] ) );
    } ?>

    <form method="post" class="dokan-form-inline offer-filter dokan-clearfix" action="">
        <div class="dokan-form-group">
            <label for="from"><?php _e( 'From:', 'dokan' ); ?></label> <input type="text" class="datepicker" name="start_date" id="from" readonly="readonly" value="<?php echo date_i18n( get_option( 'date_format' ), strtotime( $start_date ) ); ?>" />
        </div>
    ...
    dokan_offer_sales_overview( $start_date, $end_date, __( 'Sales in this period', 'dokan' ) );
}
 ...
 * @param type $heading
 */
function dokan_offer_sales_overview( $start_date, $end_date, $heading = '' ) {
    global $woocommerce, $wpdb, $wp_locale;

    $total_sales = $total_orders = $order_items = $discount_total = $shipping_total = 0;

    $order_totals = dokan_get_order_offer_data( [
        'data' => [
            '_order_total' => [
                'type'     => 'meta',
                'function' => 'SUM',
    ...
    $total_items    = absint( dokan_get_order_offer_data( [
        'data' => [
            '_qty' => [
    ...
    $total_coupons = dokan_get_order_offer_data( [
        'data' => [
            'discount_amount' => [
                'type'            => 'order_item_meta',
    ...
    $legend = apply_filters( 'dokan-seller-dashboard-offers-left-sidebar', [
    ...
    <div id="poststuff" class="dokan-offers-wrap">
        <div class="dokan-offers-sidebar offer-left dokan-left">
        ...
        <div class="dokan-offers-main offer-right dokan-right">
            <div class="postbox">
...
function dokan_sales_overview_chart_data( $start_date, $end_date, $group_by ) {
    ...
    $orders = dokan_get_order_offer_data( [
        'data' => [
            '_order_total' => [
                'type'     => 'meta',
                'function' => 'SUM',
...
function dokan_top_sellers() {
    ...
    <form method="post" action="" class="offer-filter dokan-form-inline dokan-clearfix">
        <div class="dokan-form-group">
    ...
    foreach ( $found_products as $product_id => $sales ) {
        ...
        $orders_link = apply_filters( 'dokan_offers_order_link', $orders_link, $product_id, $product_title );
